...
func leastInterval(tasks []byte, n int) int {
	...
	// Example: if A and B has the max. count and n = 2, then we should schedule like [A, B, x, A, B, x, A, B, ...],
	...
	numOfSlotsForLessFreqTasks := (maxCnt - 1) * (n - (numOfTasksWithMaxCnt - 1)) // number of x's above, minus 1 since one of the most frequent tasks like A is used
