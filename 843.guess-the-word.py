...
class Solution:
    ...
    def findSecretWord(self, words: List[str], master: 'Master') -> None:
        ...
        for _ in range(MAX_GUESSES):
            ...
            n_matches = master.guess(candidate)
            if n_matches == L:
                return
            # keep the words that have exact n_matches with candidate
            words = list(filter(lambda w: self.get_matches(w, candidate) == n_matches, words))
